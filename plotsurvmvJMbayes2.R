  ...
  function (x, estimator = c("both", "mean", "median"), 
    ...
    for (i in seq_along(which)) {
      ...
      if (!include.y) {
        ...
        lt <- x$last.time[ii]
        r. <- r[unlist(r[, 1]) >= lt, ]
        y <- 1-unlist(r.[, 3, drop = FALSE])
        y.low <- 1-unlist(r.[, 4, drop = FALSE])
        y.upp <- 1-unlist(r.[, 5, drop = FALSE])
        rng <- if (is.null(xlim)) range(x$obs.times[[ii]], x$survTimes) else xlim
        plot(x$obs.times[[ii]], unlist(x$y[[ii]]), xlim = rng, ylim = unlist(x$ry),
             xlab = xlab[i], ylab = ylab2, pch = pch, bg = bg, col = col.points)
        ff <- if (!is.null(invlink)) invlink(x$fitted.y[[ii]]) else x$fitted.y[[ii]]
        lines(x$fitted.times[[ii]], unlist(ff), col = col, lwd = lwd)
        abline(v = lt, lty = 3, col = col.abline)
        par(new = TRUE)
        matplot(r.[, 1], y, type = "l", col = col, lwd = lwd, 
        ...
        if (fill.area) {
          polygon(c(r.[, 1], rev(r.[, 1])), 
                  c(y.low, rev(y.upp)), 
                  col = col.area, border = "transparent")
          matlines(r.[, 1], cbind(y,y.low,y.upp), type = "l", col = col, 
                   lwd = lwd, lty = c(1,2,2))
        }
